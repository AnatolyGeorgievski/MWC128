...
Предлагаю рассматривать операцию редуцирования MWC128 в качестве раунд-миксера, наравне с функцией MUM-миксера. В нашей постановке задачи мы все время говорим об математически точном преобразовании, в то время как константы в MUM миксерах выбираются практически случайным образом.

```c
h = (h>>64) + (uint64_t)h * (uint128_t)MC;
```
Константа `MC` выбирается для Avalanche Effect, для достижения наилучшего лавинного эффекта, а не для модульной операции. Вопрос ставится: возможно ли синтезировать на операции MWC128 быстрый и качественный алгоритм хэша, используя константы найденные случайным образом по критериям MUM-миксера. Так мы решили модифицировать функцию FNV-1a и вместо операции умножения `h = (h^data) * MC` запишем `MUM(h^data, MC)` или `MWC(h^data, MC)` или операцию умножения с последующим редуцированием по модулю простого числа, например $2^{64} - 2^{s} +1$. Модульная операция - редуцирование по модулю простого числа может работать как миксер.

Представим еще несколько вариантов миксеров, помимо MWC.
