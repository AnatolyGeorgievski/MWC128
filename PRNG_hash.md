...
Возможно, чуть лучше с простыми числами и коллизиями обстоит дело в проекте Владимира Макарова [MUM-hash](https://github.com/vnmakarov/mum-hash). Там операция `mum` использует сложение $r_{lo}\oplus r_{hi} \bmod 2^{64}$.
...
Операция называется _folding_ ("схлопывание"). Операция годится для любых модулей. В модульной арифметике сдвиг влево или вправо можно представить через умножение на сдвиговую константу $J_n = 2^{-n} \bmod P$. Результат умножения - это два числа $(R_{lo}, R_{hi})$ - старшая и младшая части числа. Чтобы сдвинуть эти два числа на $64\cdot k$ бит, выполняется операция _folding_:
```math
(R_{lo}, R_{hi}) \leftarrow R_{lo} \cdot J_{64k} + R_{hi} \cdot J_{64k+64}
```
...
Ускорение на CPU с векторами 512 бит достигает максимума также при длине сдвига в операции _folding_ порядка 2048 бит, т.е. с шагом 32 элемента. При этом может быть использована арифметика системы команд AVX-IFMA, целочисленные векторные операции Fused-multiply-accumulate. 
