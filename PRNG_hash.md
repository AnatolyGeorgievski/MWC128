 ...
 * автор благодарит _ĀĿΞX_ (https://github.com/Alex20129), за обсуждение принципов построения mwc-хэш и помощь в тестировании

> Представляем метод синтеза некриптографической хэш-функции на алгоритмах MWC128.

- [Хэш функция на базе генератора случайных чисел](#хэш-функция-на-базе-генератора-случайных-чисел)
	- [Алгоритм MWC128-hash](#алгоритм-mwc128-hash)
	- [Алгоритм PRNG-hash](#алгоритм-prng-hash)
	- [MXM - миксеры](#mxm---миксеры)
	- [MUM - миксеры](#mum---миксеры)
	- [MWC - миксеры](#mwc---миксеры)
	- [RRX - миксер](#rrx---миксер)
	- [RNG - миксер](#rng---миксер)
	- [MRC - миксеры](#mrc---миксеры)
	- [Векторизация цикла MWC-hash](#векторизация-цикла-mwc-hash)
...
В представленной работе мы ссылаемся на опыт разработки LXM генератора [4], где авторы использовали множество типовых миксеров для придания функции генератора ГПСЧ необходимого качества, для прохождения тестов без коллизий.

В ходе разработки алгоритма мы повторили результаты [4] с использованием генератора `Xoroshiro128p` И MXM-миксера выходных значений предложенного авторами в работе. Кроме того протестировали ряд других миксеров. Исходный код подготовлен для теста SMHasher см. [xoroshiro-hash](test/xoshiro_hash.c). Для сравнения реализовали собственную версию функции [xxHash64](test/xxh64.c) переносимую. 
...
В составе алгоритма используется обратимая функция миксера: `mix` и `unmix`.

В составе алгоритма может использоваться миксер Doug Lea's. Это не самый сильный миксер, но самый простой и оптимально подошел к нашей задаче. 
```c
...
Вместо того чтобы описывать свойства миксера, я представлю класс генераторов случайных чисел. Он начинается с генератора Lehmer64 и WYrand. 
```c
...
```
Множитель выбирается так, чтобы период повтора последовательности был максимальный. Максимальный период генератора $2^{126}$. Как выбрать подобный множитель - случайно. Случайное число должно быть генератором мультипликативной группы и давать хороший лавинный эффект. Если число дает хороших лавинный эффект, то будет проходить тесты PRNG.

```c
uint64_t seed = SEED;
uint64_t wyrand() {
    seed += UINT_64_C(0x60bee2bee120fc15);
    uint128_t y = seed * (uint128_t)UINT64_C(0xa3b195354a39b70d);
    return y ^ (y >> 64);
}
```
Если из операции можно сделать трансформацию с большим периодом повтора, то это хороший кандидат на изготовление генератора последовательности псевдослучайных чисел. Некоторые числа обладают свойством генератора последовательности на операции сложения, Weyl-подобные числа. Некоторые - являются генератором поля $Z/2^{64}Z$ по умножению. От множителя требуется, чтобы он давал хороший лавинный эффект с вероятностью 50% по каждому биту. Для хорошего нелинейного смешивания можно использовать возведение в степень простого числа - генератора мультипликативной группы. 

В своем проекте, мы использовали трансформацию mum в режиме CTR-генератора (counter mode) для исследования статистических свойств констант MUM-миксера:
```c
uint64_t mum_mixer() {
    seed += gamma; // Любая Weyl-константа, 1 или -1
    uint128_t y = mum(seed, Prime);
    return y ^ (y >> 64);
}
```
В тесте инкремент `gamma` выбирается так, чтобы суммирование давало перебор всех значений по модулю $2^{64}$. Обычно для этих целей применяют Weyl-подобные константы, включая `1` и `-1`. Наиболее распространенной является константа _золотое сечение_.

Есть ряд проектов, которые применяют операцию `mum` (Multiply and Mix) в ядре хэш-функции:

* [wyhash](https://github.com/wangyi-fudan/wyhash) 
* [rapidhash](#)
* [mumhash](https://github.com/vnmakarov/mum-hash)

Операция строится по принципу
* `state ^= mum(data ^Prime1, Prime2);`
* `state ^= mum(data1^Prime1, data2^Prime2);` 

Последний вариант обладает наивысшей возможной производительностью, поскольку на одно умножение используется два элемента данных. Хотя, можно рассмотреть вариант:

* `state ^= mum(data1^Prime1, Prime2) ^ data2;` 

Возможно, чуть лучше с простыми числами и коллизиями обстоит дело в проекте Владимира Макарова [MUM-hash](https://github.com/vnmakarov/mum-hash). Там операция `mum` использует сложение $r_{lo} + r_{hi} \bmod 2^{64}$. В проекте представлен свой генератор ГПСЧ на операции MUM с запаздыванием.
```c
#define MUM_SZ 2 // размер состояния 2-16:128 ... 1024 бит
void mum_prng_update(uint64_t* state) {
...
Иногда операцию MUM хочется назвать Multyply and Rotate (MUR), тогда получаем базовую идею MurMur-хэша. В нашем случае правильно было назвать _Multiply and Reduce_, понимая под редуцированием - операцию редуцирования по модулю.

Мои тесты показали, что на операции MUM миксеры ведут себя лучше, чем XMXMX-миксеры. Операция MUM быстрее XMXMX на 64 битных процессорах. Чтобы сделать осознанный выбор мы тестировали и подбирали множители с разными Weyl-константами. 

**Обратимость операции MUM**

Умножение можно разложить, как сумму операций сдвиг-сложение с последующим редуцированием. Если операция сложения выполняется по модулю $P = 2^{64} - 1$

Операции сдвига влево 
```math
\{r_{lo}, r_{hi}\} = x\cdot 2^n \bmod 2^{64}-1
```
Эквивалентна операции `ROTL64(x, n)`


Операция `rlo + rhi` может быть интерпретирована как Lazy Reduction по модулю 
$2^{64}+1$.


Использование Lazy Reduction делает эти примитивы непригодными для криптографии (например, для MAC или шифрования), так как отсутствие полного приведения по модулю может создавать неочевидные алгебраические уязвимости и коллизии. Чтобы получить точное и обратимое значение, нужно результат операции корректировать.

**Сравнение схем редукции MUM (MUltiply-adn-Mix)**

| Операция | Формула | Интерпретация модуля | Обратимость | Производительность |
|----------|---------|----------------------|-------------|-------------------|
| **MUM (XOR)** | `rlo ^ rhi` | GF(2)/($x^{64} + 1)$ | Условная | ⭐⭐⭐⭐⭐ |
| **MUM (ADD)** | `rlo + rhi` | Lazy $2^{64} - 1$ | Условная | ⭐⭐⭐⭐ |
| **MUM (SUB)** | `rlo - rhi` | Lazy $2^{64} + 1$ | Условная | ⭐⭐⭐⭐ |
| **MOD (pseudo-Mersenne)** | `rlo + M·rhi` | $2^{64} - M$ | Полная | ⭐⭐⭐ |

**Примеры реализации**
```cpp
// XOR-редукция GF(2)/($x^{64} + 1)
uint64_t mum_xor(uint64_t a, uint64_t b) {
    __uint128_t p = (__uint128_t)a * b;
    return (uint64_t)p ^ (uint64_t)(p >> 64);
}
// ADD-редукция (Lazy reduction mod 2^64-1)
uint64_t mum_add(uint64_t a, uint64_t b) {
    __uint128_t p = (__uint128_t)a * b;
    return (uint64_t)p + (uint64_t)(p >> 64);
}
```
...
Наиболее общее определение операции MWC можно ввести через операцию Montgomery-friendly reduction, когда одновременно выполняется выполняется сдвиг на слово и редуцирование по модулю. Операция модульного умножения Montgomery-friendly будет давать результат\
$r = A\cdot B \cdot 2^{-n} \bmod P$.

Предлагаю рассматривать операцию сдвига и редуцирования MWC в качестве миксера, наравне с функцией MUM-миксера. В нашей постановке задачи мы все время говорим об математически точном преобразовании, в то время как константы в MUM миксерах выбираются практически случайным образом по критерию достижения лавинного эффекта.

```c
h = ((h>>64) + (uint64_t)h * A1) * MC;
```
Константы `MC` могут выбираться для Avalanche Effect, для достижения наилучшего лавинного эффекта. При этом операция MWC миксера - обратима. Вопрос ставится: возможно ли синтезировать на сдвиговой операции MWC128 быстрый и качественный алгоритм хеша, используя константы найденные случайным образом по критериям MUM-миксера. Так мы решили модифицировать функцию FNV-1a и вместо операции умножения `h = (h^data) * MC` запишем `MUM(h^data, MC)` или `MWC(h^data, MC)` или операцию умножения с последующим редуцированием по модулю простого числа, например $2^{64} - 2^{s} +1$. Модульная операция - редуцирование по модулю простого числа может работать, как миксер.

Представим несколько вариантов миксеров MWC.

Введем обозначения: $[X]^k = \left\lfloor {X \over 2^k}\right\rfloor$, $[X]_k = X \bmod 2^k$

Представляем варианты миксера на шаге редуцирования различных простых чисел [[Plantard](https://thomas-plantard.github.io/pdf/Plantard21.pdf)]:
```math
\begin{align}
h &\leftarrow [h]_n + [h]^n &\quad\text{Mersenne}&\quad P=2^{n} -1\\
h &\leftarrow [h]_n + [h]^n2^s\mp [h]^n &\quad\text{generalized Mersenne}&\quad P=2^{n} -2^{s} \pm 1\\
h &\leftarrow [h]_n + [h]^n\cdot M &\quad\text{pseudo Mersenne}&\quad P=2^{n} -M, M<2^{n/2}\\
h &\leftarrow [h]^n + [h]_n\cdot M &\quad\text{MWC, Rizel}&\quad P=M\cdot 2^{n} -1, M<2^n\\
h &\leftarrow [h]^n - [h]_n\cdot M &\quad\text{NTT, Proth}&\quad P=M\cdot 2^{n} +1, M<2^n\\
\end{align}
```
Заметим, что шаг миксера для _NTT-friendly_ простых чисел выполняется над числами со знаком, в то время как _MWC_ (Простые числа Ризеля) на числах без знака. В коде _NTT-friendly_ итерация обозначается, как _signed MWC_. В литературе редукция MWC обозначается, как _Montgomery-friendly_, совмещает сдвиг и редуцирование по модулю простого числа.

В проекте мы тестируем миксеры: _MWC_, _NTT-friendly (signed MWC)_, _Mersenne_. 

Операцию хэша можно представить в виде полинома, в котором участвует константа смешивания $\alpha$.
```math
\begin{align}
h &= d_0 + d_1 \beta + d_2 \beta^2 + ... + d_{k-1} \cdot \beta^{k-1}\\
h &= d_0\alpha^{k-1} + d_1 \alpha^{k-2} + ... + d_{k-2} \alpha + ... + d_{k-1} \quad \bmod P
\end{align}
```
Операции сложения и умножения можно выполнять в арифметике GF(2), с заменой операции сложения на XOR, а умножения - на полиномиальное умножение (CLMUL, умножение без переносов). 

Для алгоритма MWC-hash мы выбираем $\alpha \beta \equiv 1 \bmod P$.

Величина $\alpha$ должна обладать некоторым свойством - максимальный период мультипликативной группы по модулю P. Для простых чисел Прота, NTT-friendly, максимальный период может быть найден как $\alpha^{(P-1)/w} \equiv 1 \bmod P$. Для чисел Ризеля, максимальный период при $w=2$. По модулю $P=2^{64}$ максимальный период $2^{62}$.




В обобщенном виде итерацию с произвольным шагом можно представить одним из вариантов:



```math
\begin{align}
h &\leftarrow (h \oplus d_i) M_c ~\mod P  & \text{ -- шаг FNV-1a}\\
h &\leftarrow  h \cdot M_c \oplus d_i ~\mod P  & \text{ -- шаг FNV-1~~}\\
h &\leftarrow  ([h]_n \cdot M_1 + [h]^n \cdot M_2) \oplus d_i  & \text{ -- шаг MWC~~~}\\
h &\leftarrow  ([h]_n \cdot 2^{64-n} + [h]^n)\cdot M_c \oplus d_i  & \text{ -- шаг MWC~~~}\\

\end{align}
```
Предложения по модификации функции [FNV-1a](https://www.rfc-editor.org/rfc/rfc9923.html)(Glenn Fowler, Landon Curt Noll, and Kiem-Phong Vo) сводится к замене константы на простое число с хорошим лавинным эффектом, и замене модуля $P=2^{64}$ на операцию редуцирования по модулю простого числа, после умножения $64 \times 64 \to 128$ бит. Помимо модульной операции, можно примерять различные операции смешивания типа MUM.
<!--
Вариант шага xxHash-64 строится немного иначе
```math
\begin{align}
h &\leftarrow h + d_i P_1\\
h &\leftarrow ROTL(h,R)\cdot P_2
\end{align}
```
Особенностью является использование двух констант для смешивания. 
-->

Интересные частные случаи миксера для генерализации MUM, multiply and mix:
* `h = rlo ^ rhi` -- _Multiply and mix_, 
* `h = rlo + rhi` -- _Multiply with carry mix_,
* `h = rlo - rhi` -- _signed Multiply with borrow mix_
* `h = rlo - rhi + (rhi<<S)` -- _NTT-friendly_; $P = 2^{64}-2^{s}+1$
* `h = rlo ^ rhi ^ (rhi<<S)` 
* `h = rlo ^ rhi ^ (rhi>>S)` -- _MXM_, _multiply with Xor mixer_, 
* `h = rlo ^ ROTL(rhi, R)` -- _MRX_, _multiply with Rotate-Xor mixer_, 
* `h = ROTL(rlo ^ rhi, R)` -- _MXR_, _multiply with Xor-Rotate mixer_, 
* `h =(rlo + rhi) ^ ROTL(rhi, R)` -- _ARX_, _Add-Rotate-Xor mixer_, 

Эксперименты с этими миксерами представлены в виде вариантов хэша [FNV-1a](test/fnv1a.cpp). 


Заметим, непременным условием хорошего миксера является возможность построения на итерации генератора последовательности псевдо-случайных чисел с максимальным периодом. Критерием хорошего миксера является простота числа `M` и баланс нулей и единиц. Множитель `M` должен обладать максимальным порядком мультипликативной группы $Z/2^{64}Z$. Дополнительные требования на качество констант `Linear Complexity` и прохождение тестов `Bit Independence Criteria` на кратных периодах $2^n$, $n>28$.

Генератор случайных чисел с выходом $n$-бит ($y = [s]_n$) на MWC128 - миксере:
```math
\begin{align}
d_i &\leftarrow d_{i-1} + \gamma\\
s   &\leftarrow ([s]_n \cdot M_1 + [s]^n) \cdot M_c \oplus d_i
\end{align}
```
где $\gamma$ - Weyl- подобная аддитивная константа, опция. Для достижения максимального периода, константы M1 и Mс должны быть взаимно простыми. Базовая константа $M_c$ для Avalanche эффекта может выбираться, как простое число c максимальный порядком мультипликативной группы $2^{62}$ и балансом нулей и единиц. Максимальный порядок итерации MWC128 $\approx 2^{127}$.

## RRX - миксер

Мы хотим рассмотреть обратимый миксер на операциях вращения ROR и XOR. Множественные операции вращения ROR можно представить как умножение на матрицу аффинного преобразования в поле GF(2). Операция ROR порождает матрицу - циркулянт, в которой все строки сдвинуты. Матрица полностью определена первой строкой или образующим полиномом p(x) в первой строке.

Произведение матрицы на вектор можно заменить на операцию полиномиального умножения образующего полинома с последующим редуцированием:
```math
(a(x) \odot p(x)) \pmod{x^{64} +1}
```

Это делает класс RRX-миксеров похожим на MUM, но с заменой операций умножения на полиномиальное умножение, умножение без переносов. Так же как в случае MUM-миксера мы будем искать константу (полином) для умножения, которая дает хороших эффект смешивания во всех разрядах.

Для начала мы можем рассмотреть скрамблер вида 
```math
x \mapsto x \oplus (x \lll i) \oplus (x \lll j) 
```
и набор скрамблеров Sum0, Sum1 из стандарта SHA-256/-512.

Далее можно предложить общий метод представления подобных скрамблеров с произвольным числом операций ROR. Каждое вращение будет обозначено одним битом в образующем полиноме. Полином получается методом подставить единицу в функцию RRX-миксера, если она представлена операциями ROR. 

**Метод обращения RRX**

Операция RRX-миксера порождает матрицу - циркулянт. Обратная циркуляционная матрица тоже циркулянт. 

Циркулянт размера $n\times n$ полностью определяется первой строкой (или первым столбцом):
    $c = [c_0, c_1, c_2, …, c_{n-1}]$
Матрица соответствует полиному
```math
p(x) = c_0 + c_1 x + c_2x^2 + … + c_{n-1}x^{n-1}
```
Обратная матрица тоже циркулянт, соответствует полиному q(x) в кольце полиномов:
```math
p(x) \cdot q(x) ≡ 1 \pmod{x^n − 1}
```
Это справедливо для любой арифметики, в т.ч. арифметики Галуа. При переходе к арифметике Галуа, операцию `-` заменяем на `+` и коэффициенты полинома представляем в поле GF(2).

Чтобы найти обратное преобразование надо найти обратный полином в кольце полиномов, $q = p^{-1}$:
```math
p(x) \odot q(x) \equiv 1 \pmod{x^{n} +1}
```
Классический метод нахождения обратного полинома основан на обобщенном алгоритме Евклида. Но мы предлагаем более простой способ. Нужно возводить полином p(x) в степень пока не будет найден обратный. Степень может варьироваться от 1 до n-1. 

Для возведения полиномов в степень нам понадобится операция умножения полиномов с редуцированием `pmull` и алгоритм возведения в степень `p_pow` на её основе.  Используется бинарное возведение в степень (exponentiation by squaring).
```c
for (ex = 1; ex<64; ex+=2) {
	p_inv = p_pow(p, ex);
	if ( pmull(p, p_inv) == 1) break;
}
```
Не все полиномы обратимы. При выборе обратимого полинома следует использовать: нечётное число ненулевых коэффициентов (вес Хэмминга нечётный). Отсуствие общих делителей $GCD(p(x), x^n+1) ==1$, тоже верный признак. Но проверять GCD() - избыточно, поскольку предлагаемый алгоритм поиска, основанный на возведении в степень, в противном случае не даст результата. Алгоритм можно оптимизировать чтобы проверять степени $2^e-1$.

Если полином выбирается, как случайное число, то фильтр
`popcnt(p)&1==1` -- (нечетность числа коэффициентов), самый быстрый способ отсева кандидатов. 

Полином p(x) обратим ⇔ существует q(x) такое, что $p(x) · q(x) ≡ 1 \pmod{x^n + 1}$
⇔ $gcd(p(x), x^n + 1) = 1$

При синтезе произвольного скрамблера можно использовать критерий аналогичный MUM: баланс единиц и ноликов в каждом байте.

Произвольный скрамблер выражается в две инструкции и полностью описывается набором коэффициентов полинома p(x), битами числа p;
```c
uint64_t scrambler(uint64_t x, uint64_t p){
  return pmull(x, p);// carry-less mul & XOR
}
```
Для хорошего обратимого скрамблера найдется обратное число `p_inv`, которое задает обратное преобразование.

реализация метода синтеза и обращения функции скрамблера на полиномиальном умножении см. [pmull_mixer.c](test/pmull_mixer.c)


**Изоморфизм конечных полей**

Все конечные поля одной и той же степени изоморфны. В частности, любые два представления поля GF(2ⁿ), построенные по разным неприводимым полиномам степени n изоморфны.  
...
В контексте циркулянтных преобразований особенно полезна операция _сопряжения_ (англ. conjugation):
```math
p_{new}(x) = r(x) · p(x) · r^{-1}(x)  \pmod{x^{64} + 1}
```
где $r(x)$ — любой обратимый полином (т.е. $gcd(r(x), x^{64} + 1) = 1$).  
...
Шаг алгоритма на произвольном генераторе ГПСЧ можно синтезировать следующим образом
```math
\begin{align}
s   &\leftarrow (rng(s) \oplus d_i)  \cdot M_c\\
s   &\leftarrow rng(s) \oplus d_i  \cdot M_c\\
s   &\leftarrow rng(s) \cdot M_c \oplus d_i\\
\end{align}
```
где `rng()` - шаг выбранного алгоритма ГПСЧ. И конечно мы решили опробовать методы FNV-1a с несколькими генераторами, в частности Xorshift и Xoroshiro. 

Генератор случайных чисел можно векторизовать за счет ротации состояния однотипных RNG генераторов, как это предложено в проекте MUM-hash, где под операцией MUM понимается умножение $64\times 64 \to 128$ с редуцированием в 64 бит.
```c
#define MUM_SZ 2 // размер состояния 1-16:64 ... 1024
void mum_rng(uint64_t* state) {
    for (int i = 0; i < MUM_SZ - 1; i++)
        state[i] ^= mum (rng(state[i + 1]), Primes[i]);
    state[MUM_SZ - 1] ^= mum(rng(state[0]), Primes[MUM_SZ - 1]);
}
```
<!--
```math
\begin{align}
s   &\leftarrow (rng_{i+1}(s) \oplus d_i)  \cdot M_c\\
\end{align}
```
-->
...
<!--
Следует так же обратить внимание на рандомизацию Middle-Square
```c
uint64_t msrand() {
	x = x*x + rng(); 
	return x = (x>>32) | (x<<32);
...
```
[[1704.00358](https://arxiv.org/pdf/1704.00358)] Middle-Square Weyl Sequence RNG

Метод Middle-Square RNG позволяет предложить вариант двойного шага, как в `rapidhash`
-->


## MRC - миксеры

Давайте представим, что умножение на большую константу может быть хорошим миксером. Идея MRC-миксера происходит из китайской теоремы об остатках (CRT) из алгоритма RNS to MRC- mixed radix conversion, преобразование из системы остаточных классов в позиционную систему. Пусть a1, a2 независимо посчитанные числа от входного потока данных по двум простым модулям p1, p2. 
```c
uint128_t mrc_mix(uint64_t a1, uint64_t a2) {
    const uint64_t y1 = mod_inverse(p2 % p1, p1);
    const uint64_t y2 = mod_inverse(p1 % p2, p2);
    return a1 * p2 * y1 + a2 * p1 * y2;
}
```
Подобный миксер можно использовать для увеличения разрядности MWC64-hash, при этом два (и более) модулей MWC могут считаться параллельно. 

* см. Residue Number System (RNS) -- система остаточных классов
* см. Numeric Theoretic Transform (NTT) -- еще один метод смешивания, подобный дискретному Фурье преобразованию (DFT).

Обе трансформации - обратимы. Метод увеличения разрядности будем называть RNS-MWC-hash. Метод смешивания _бабочками_ будем называть NTT-hash. Оба метода пригодны для реализации на GPU.

## Векторизация цикла MWC-hash

Основная операция векторного хеш называется _folding_ ("схлопывание"). В модульной арифметике сдвиг влево или вправо можно представить через умножение на сдвиговую константу $J_n = \alpha^{n} \bmod P$. Основание сдвиговой константы для простоты понимания можно принять 2. Но на практике основание $\alpha$ - любое число по модулю, которое является генератором мультипликативной группы с максимальным периодом. Я бы предложил выбирать основание таким образом, чтобы сдвиговые константы одновременно удовлетворяли двум критериям: генератор мультипликативной группы по модулю P и максимальный порядок по умножению в кольце Z/2^64Z. Качество сдвиговой константы в качестве миксера можно оценивать по балансу единичных и нулевых бит HammingWeight и тесту Bit Independence, который можно применить отдельно к операции _folding_. 

Результат умножения - это два числа $(R_{lo}, R_{hi})$ - старшая и младшая части числа. Чтобы сдвинуть эти два числа на $n = 64\cdot k$ бит, выполняется операция _folding_(n):
```math
Fold(R_{lo}, R_{hi}, n): (R_{lo} + R_{hi}2^{w}) \leftarrow R_{lo} \cdot J_{n} + R_{hi} \cdot J_{n-w}
```
Эта операция выполняется над векторами $n=2k\cdot w$- элементов и не требует редуцирования по модулю. Операция редуцирования по модулю выполняется после завершения цикла и после выравнивания сдвигов элементов вектора, т.н. отложенная операция редуцирования. Операцию сложения с выравниванием сдвигов назовем _merge_. Именно такой подход мы предлагаем использовать на GPU для параллельного расчета хеш-функции от тензорных данных, векторизация выполняется по warp-группе, 32 элементов. 

Ускорение на CPU с векторами 512 бит достигает максимума также при длине сдвига в операции _folding_ порядка 2048 бит, т.е. с шагом 32 элемента. При этом может быть использована векторная арифметика системы команд AVX-IFMA, целочисленные векторные операции Fused-multiply-accumulate. 

Операция _folding_ требует тщательной оптимизации и использования ассемблерных вставок для оптимизации умножения, в проекте мы представляем переносимый (_portable_) вариант алгоритма с явной векторизацией цикла.

Приведем пример построения векторного алгоритма для разрядности 64-бит под использование модулей:
* $P=A_0 2^{32} \pm 1$ _MWC64_ или _signed MWC64_ (NTT-friendly) модули
* $P=2^{64}-2^{32}\pm 1$(Goldilocks and Three Bears) - используется в пост-квантовой криптографии и ZKP
  
Листинг ядра параллельного расчета хеш функции (псевдо-код), fold x4:
```c
// вычисление сегмента
void hash_segment(uint8_t* data, size_t len, uint128_t* out){
	uint128_t h[4] = {0};
	// параллельно в группе x4
	while (len >= 32) {
		h[0] = fold(h[0] ^ GET_U64(data, 0), 32);
		h[1] = fold(h[1] ^ GET_U64(data, 8), 32);
		h[2] = fold(h[2] ^ GET_U64(data,16), 32);
		h[3] = fold(h[3] ^ GET_U64(data,24), 32);
		data+=32, len-=32;
	}
	// слияние по группе (merge)
	h[0] = fold(h[0], 16) + h[2];
	h[1] = fold(h[1], 16) + h[3];
	h[0] = fold(h[0],  8) + h[1];
	*out = h[0];
...
```
В ядре не применяется модульная операция, можно использовать lazy-reduction - отложенную операцию модульного редуцирования.

Листинг процедуры расчета хеш функции (псевдо-код):
```c
size_t n_segments = data_len / SEGMENT_SIZE;
uint128_t segment[n_segments];
for (size_t k=0;k<n_segments; k++) {
	hash_segment(data, SEGMENT_SIZE, &segment[k]);
	data+= SEGMENT_SIZE; 
}
uint64_t h = seed;
// слияние по сегментам  (merge)
for (size_t k=0;k<n_segments; k++) {
	h = fold(h, SEGMENT_SIZE) + segment[k];
}
size_t len = data_len%SEGMENT_SIZE;
// выравнивание на слово (опционально)
while (len >= 8) {
	h = fold(h ^ GET_U64(data, 0), 8);
	data+=8, len-=8;
}
// обработка остатка, по байтам (опционально)
while (len >= 1) {
	h = fold(h ^ GET_U8(data, 0), 1);
	data+=1, len-=1;
}
// модульная операция и миксер
return mixer(h);
```
<!-- Чтобы уменьшить число умножений на элемент данных в цикле мы можем выбрать константу базового сдвига $J_{n} \equiv 1 \bmod P$.--> При распределении вычислений между ядрами финальная функция `mixer()` выполняется со стороны хоста после слияния хешей от вычислительных ядер. Слияние по ядрам выполняется точно так же, через умножение на сдвиговые константы: `fold(h1, n) + h2`. Добавление `seed` можно выполнять после вычисления хэш ядрами, так же через слияние сегментов. В алгоритме мы можем переопределить оператор `+` и `^` под конкретную модульную операцию. 
...
* [CRC64-hash](test/crc64_hash.c) Алгоритмы представлены с векторизацией и Folding x4 на операциях CLMUL - умножение без переносов, полиномиальное умножение в поле GF(2) для нескольких полиномов степени 64, 32. Представлены таблицы сдвиговых констант для CRC-32,CRC-32B, CRC-32/K (Koopman), CRC-32C (Castagnoli), CRC-32/Q, CRC-24/OPENPGP
...
Заметим, тест xoroshiro выполняется для побайтовой операции, в то время как шаг алгоритмов слово или вектор 64 бит.
...
* [MWC128-hash 64-bit](test/mwc128_hash.cpp) -- использован MUM-миксер
* [signed MWC64 64-бит](test/mwc64s_hash.cpp)
* [XXH64-hash 64-bit](test/xxh64.cpp) -- в алгоритм добавлен входной миксер `avalanche(seed)` для прохождения теста
* [FNV1a-64](test/fnv1a.cpp) -- семейство алгоритмов, добавлен входной миксер на SEED и выходной миксер типа MUM. Ряд вариантов используют MUM раунд-миксер.
* [CRC-64](test/crc64_hash.cpp) -- алгоритмы CRC-64/NVME, CRC-64/XZ, CRC-64/ECMA и другие с разными полиномами степени 64. Реализованы оба варианта CRC-64: с прямым и отраженным порядком бит. В частности в ZFS для хеш-таблиц используется алгоритм CRC-64/ECMA с отраженным порядоком бит.
* [Fletcher's checksum](test/flatcher.cpp) -- контрольная сумма на сложении по модулю простого числа.
[Fletcher4](https://people.freebsd.org/~asomers/fletcher.pdf) - вариант алгоритма по модулю $2^{64}$ используется в ZFS на блоках 128K. 


> Добавление входного и выходного миксера, дающего лавинный эффект смешивания бит, непременное условие прохождения теста. В практических задачах, таких как хэш таблицы, миксер можно исключить или заменить на облегченную версию. Условие прохождение теста - `Avalance` и `Bit Independence` требует, чтобы измерение одного бита влияло на все биты выходного значения.
...
| [MWC128-128](test/mwc128-128-smhasher3.txt)| PASS   | 188/188  | || 31.70
