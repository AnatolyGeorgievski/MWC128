...
```
Где константа `MC` выбирается для Avalanche Effect, для достижения наилучшего лавинного эффекта, а не для модульной операции. Вопрос ставится, возможно ли синтезировать на операции MWC128 быстрый и качественный алгоритм хэша, используя константы найденные случайным образом по критериям MUM-миксера. Так мы решили модифицировать функцию FNV-1a и вместо операции умножения `h = (h^data) * MC` запишем `MUM(h^data, MC)` или `MWC(h^data, MC)` или операцию умножения с последующим редуцированием по модулю простого числа, например $2^{64} - 2^{32} +1$. Модульная операция - редуцирование по модулю простого числа может работать как миксер.
...
\end{align}
```
Заметим, что шаг миксера для _NTT-friendly_ простых чисел выполняется над числами со знаком, в то время как _MWC_ (Простые числа Ризеля) на числах без знака. В коде _NTT-friendly_ итерация обозначается, как _signed MWC_. В литературе редукция MWC обозначается, как Montgomery-friendly, совмещает сдвиг и редуцирование по модулю простого числа.

В проекте мы тестируем миксеры: _MWC_, _NTT-friendly (signed MWC)_, _Mersenne_. 
В обобщенном виде итерацию с произвольным шагом можно представить как
```math
h \leftarrow [h]^n\cdot M_1 + [h]_n\cdot M_2
```
-- в таком виде операция называется _folding_. Операция позволяет перейти к векторному вычислению хэш. Операции сложения и умножения можно выполнять в арифметике GF(2), с заменой операции сложения на XOR, а умножения - на полиномиальное умножение (CLMUL, умножение без переносов). 
...
* `h = rlo - rhi + (rhi<<S)` -- _NTT-friendly_; $P = 2^{64}-2^{s}+1$, _Goldilocks_
* `h = rlo ^ rhi ^ (rhi<<S)` -- _generalized MUM_
* `h = rlo ^ rhi ^ (rhi>>S)` -- _MXM_, _multiply with Xor mixer_, 
* `h = rlo ^ ROTL(rhi,R)` -- _MXR_, _multiply with Xor-Rotate mixer_, 
...
Давайте представим, что умножение на большую константу может быть хорошим миксером. Идея MRC-миксера исходит из китайской теоремы об остатках (CRT) из алгоритма MRC- mixed radix conversion. Пусть a1, a2 независимо посчитанные числа от входного потока данных по двум простым модулям p1, p2. 
```c
...
Операция называется _folding_ ("схлопывание"). В модульной арифметике сдвиг влево или вправо можно представить через умножение на сдвиговую константу $J_n = 2^{-n} \bmod P$. Результат умножения - это два числа $(R_{lo}, R_{hi})$ - старшая и младшая части числа. Чтобы сдвинуть эти два числа на $n = 64\cdot k$ бит, выполняется операция _folding_(n):
```math
(R_{lo} + R_{hi}2^{w}) \leftarrow R_{lo} \cdot J_{n} + R_{hi} \cdot J_{n-w}
```
Эта операция выполняется над векторами $k$- элементов и не требует редуцирования по модулю. Операция редуцирования по модулю выполняется после завершения цикла и после выравнивания сдвигов элементов вектора, т.н. отложенная операция редуцирования. Операцию сложения с выравниванием сдвигов назовем _merge_. Именно такой подход мы предлагаем использовать на GPU для параллельного расчета хэш-функции от тензорных данных, векторизация выполняется по warp-группе, 32 элементов. 
...
Операция _folding_ требует тщательной оптимизации и использования ассемблерных вставок, в проекте мы представляем переносимый (_portable_) вариант алгоритма с явной векторизацией цикла.
