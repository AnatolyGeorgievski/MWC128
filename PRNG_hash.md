...
Предлагаю рассматривать операцию редуцирования MWC128 в качестве миксера входных и выходных данных, наравне с функцией MUM-миксера.

```
h ^= (h>>64) + (uint64_t)h * (uint128_t)MC;
```
Где константа `MC` выбирается для Avalanche Effect, для достижения наилучшего лавинного эффекта, а не для модульной операции. 
