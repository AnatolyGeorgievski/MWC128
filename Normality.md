...
Сначала нужно доказать, что метрика _сложность_ предложенная в предыдущем параграфе достаточно точно отражает суть скрамблера, который не меняет _сложность_ генератора, но улучшает однороденость распределение в тесте **Leading/Trailing Zeros**. Для этого мы прогоняем через тест обычный счетчик `x = x+1`, который дает референсное значение (2.089) при полном прогоне $2^{32}$ итераций. Хороший скрамблер не меняет _сложность_ распределения! Хороший скрамблер должен улучшать прохождение теста HammingWeight2.
...
```math
\begin{align}
x &\mapsto  (x\ggg 1) \oplus (x \ggg 8) \oplus (x\gg 7), &\text{ - Sigma0}\\
x &\mapsto  (x\ggg 19) \oplus (x \ggg 61) \oplus (x\gg 6), &\text{ - Sigma1}\\
x &\mapsto  (x\ggg 28) \oplus (x\ggg 34) \oplus (x \ggg 39), &\text{ - Sum0}\\
x &\mapsto  (x\ggg 14) \oplus (x \ggg 18) \oplus (x \ggg 41), &\text{ - Sum1}
\end{align}
```
