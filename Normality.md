...
Заметим, что последовательность может не содержать степени двойки, но может содержать повторяющиеся паттерны. Повтор периода с установленной точностью считается провалом теста. Периоды должны иметь случайное нормальное распределение. Средний период не должен быть менее установленного порогового значения. Для генераторов MWC период решетки оценивается в (P-31/32) для 64 битной версии и (P-63/64) для версии 128 бит. Оценка периода повтора паттерна 64 бита: MWC128 > 2^{122}. Реально такие периоды мы не можем проверить. Проверка выполняется для счетчика итераций от 2^{32} и выше по установленной маске 32 бита.
