...
Для тестирования и сравнения генераторов предлагают методику *Hamming–Weight Dependencies* [HWD](https://prng.di.unimi.it/hwd.php). В публикации авторы ссылаются на ряд тестов "Hamming–Weight" из пакета, присутствующие в наборе Crush-тестов TestU01. Метод тестирования 64 битных генераторов, сводился к отдельному тестированию старшей и младшей части.
...
Для генерации однородного распределения float U(0,1) мы рекомендуем использовать MWC64x. Для генерации в формате double следует использовать генераторы 128 бит, такие как MWC128 и Xoroshiro128.
