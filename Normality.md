...
```
В таком виде мы его применяем для генерации однородного и гауссова распределения. За одну итерацию MWC64 не полностью меняет состояние, итерация эквивалетна делению/сдвигу $x \mapsto x\cdot 2^{-32} \bmod (A_0\cdot 2^{32}-1)$. Чтобы обновились все 64 бита, итерацию надо применять дважды.
...
Для финальной верификации применяются собственые реализации критериев AD, KS, χ² к большим выборкам ($10^{8}$–$10^{9}$ итераций). Поверх теста применяется цикл прогона по начальным значениям SEED с использованием шага $2^32$ значений, цикл состоит из $512$ раз.
...
2. **Difficulties** Данный тест заимствован из способа вычисления сложности шары в майнинге ... и может использоваться, как интегральный аналог теста `Leading Zeros`. Значения, которые попадают в категорию N-бит, суммируются со своей сложностью. Сложность вычисляется, как вещественное число по формуле:
```c
...
3. **Hamming-Weight Distances** Тест заимствован у разработчиков Xoroshiro128. 
...
Далее мы выбираем категории, в нашем примере мы разбили сложности по 4 бита, по степеням. 
...
Мы подобрали несколько линейных и нелинейных скрамблеров из числа операций, которые применяются при построении хэш-функций. В частности, мы рассмотрели скрамблеры предложенные в для хэш функции на модульной арифметике [[2023/1025](https://eprint.iacr.org/2023/1025.pdf)].
...
```
<!--
По результатам нашего теста ограниченно годными мы признаем скрамблер `Gray`, но не рекоменудем его к применению в составе ГПСЧ. Остальные методы из нашего списка (линейные, нелинейные, битовые и байтовые перестановки) проходят тест _сложности_. Скрамблеры, использованные в составе Xoroshiro128 тест НЕ проходят, и скрамблеры из SHA512 тест НЕ проходят.
-->
