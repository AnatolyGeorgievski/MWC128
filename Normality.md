...
Понятия [Observability](https://en.wikipedia.org/wiki/Observability) и [Controllability](https://en.wikipedia.org/wiki/Controllability) введены Р. Е. Калманом для линейных динамических систем, и применяется для State-Space Model[1] в теории управления. Математически критерий Управляемости (в теории управления) определяются через соотношение матрицы B и A, путем построения подпространства Крылова, где B может снизить размерность пространства (когда ранг матриц $\{A^kB\}$ оказывается ниже рагнга матриц $\{A^k\}$). В этом случае система не полностью управляема. При каноническом разложении мы получаем блочно-диагональный вид матрицы генерации и часть вектора состояния не будет зависеть от входных данных. 
...
В этом контексте мы рассматриваем часть состояния, как не контролируемое, поведение компоненты будет определяться только начальным условием. Стохастическая компонента вектора состояния - это та, которую мы не можем контроллировать через внешний порт. Таким образом мы говорим про два генератора, выход стохастической компоеннты используется для генерации внутреннего состояния. В теме генераторы ГПСЧ такая конструкция будет соответствовать комбинированному генератору. Понятие порт внешнего управления не применяется к ГПСЧ, только к хэш-функции, в которой присуствует подмешивание данных с порта $u(t)$ через матрицу $B$. 