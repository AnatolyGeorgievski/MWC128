...
4. Разработка алгоритма подбора заданий на основе вычислительного графа с архитектурой SPONGE (губка с режимом генерации, подобно хеш-генерации).
...
Мы отдельно рассматриваем левые и правые отклонения, относительно центра нормального распределения $F(x,\theta)$, $D_{n}^{+}$ и $D_{n}^{-}$. В критерии Кёйпера используется статистика вида: 
$V_{n}=D_{n}^{+}+D_{n}^{-}$.
...
```
> Магия числа 12 мне не ясна. Очевидно это какая-то поправка, подгон для согласования критерия при малых $n$.
...
Существуют пакеты тестирования генераторов, которые позволяют сравнить в различных тестах. 
...
```
Заметим, что в метод MWC64 добавлена операция XOR: `x ^ (x>>32)`. Я бы назвал это словом "рандомизатор" или "скрамблер", эта операция позволяет улучшить прохождение некоторых тестов. С той же целью, в метод `Xoroshiro64` добавлено умножение результата итерации на большое простое число. Авторы метода предлагают несколько таких "рандомизаторов", отчего алгоритмы отмечаются звездочками и плюсиками. 

## Генераторы последовательности псевдослучайных чисел
...
2. **Перестановка (permutation)**: Мы предполагаем использование операций над тензором: перестановки, группировки и нормировки. Рассматриваем возможность проецирования на гиперплоскость. Задача — найти гиперплоскость, на которой данные будут представлены в 2D и обладать контрастным распределением (например, с помощью PCA или t-SNE для снижения размерности).
...
*Дополнение*: Алгоритм можно расширить на машинное обучение, интегрируя автоэнкодеры для выявления аномалий в распределениях. Это позволит автоматизировать раннее выявление нерабочих режимов оборудования.
