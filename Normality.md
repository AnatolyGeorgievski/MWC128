...
- [Критерий нормальности Колмогорова-Смирнова](#критерий-нормальности-колмогорова-смирнова)
  ...
  - [Рештчатая структура ГПСЧ](#рештчатая-структура-гпсч)
...
4. Применить BB-алгоритм для нахождения кратчайшего вектора в дуальной решётке (спектральный тест).

> В этом месте мы хотим отойти от предложенного плана и выстроить некоторые аналогии из теории Stochastic Time Series, Моделирование стахостических временных последовательностей. Наша задача научиться предсказывать шаг функции и предсказывать начальную точку, для поиска паттернов в тесте *Trump aces*.
...
3. Предсказание шага выборки одномерного генератора может выполняться в терминах ARIMA (autoregressive integrated moving average) - в модели используется последовательность входных и выходных значений с накоплением результата. Особенность использования метода - совместимость с ARX- структурами обеспечивается за счет переопределения операции умножения и модульной арифметики. ARIMA- одномерная модель, в то время как нас интересует многомерная однородность. 
4. Для моделирования генератора в цепи управления шагом мы используем аналогии из теории порт-Гамильтоновых систем и State-Space Model, систем содержащих _вращение_ без диссипации. Вращение описывается матрицей генерации с симплектичесокй структурой или со структурой матрицы-циркулянта. Вращение может выполняться в кольце полиномов (связь с циклотомическими полиномами). 
...
**Auto-regressive-часть в модульной арифметике**
...
что представляет собой _авторегрессию порядка_ $k$ в кольце $\mathbb{Z}/m\mathbb{Z}$. Поэтому AR-часть (а при отсутствии шума и внешнего управления — чистая AR-модель) здесь естественна и *полностью рабочая* основа для точного предсказания следующих состояний линейного генератора.
...
Существует несколько эквивалентных представлений матрицы-компаньона. Мы будем использовать *транспонированную форму*, которая наиболее удобна для представления линейных рекуррентных соотношений и LFSR:

Для monic многочлена
```math
p(x) = x^d + c_{d-1}x^{d-1} + \dots + c_1 x + c_0
```
...
Результат разложени RCF — это блочно-диагональная матрица, где каждый блок — сопровождающая матрица неприводимого многочлена - делителя характеристического многочлена. 
...
Решётка, порождённая этим блоком, выделяется в соответствующем $d_i$-мерном подпространстве после преобразования подобия $P^{-1}AP$. Диапазоны бит состояния генератора после изменения базиса соответствуют этим независимым циклическим компонентам.
