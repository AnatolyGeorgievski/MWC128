...
Раунд ChaCha можно представить как 
    S = P(S) -- операция смешивания слов и бит, состоящая из ARX-структур
Обратное преобразование раунда
    S = P^{-1}(S) -- обратная операция смешивания слов и бит

Обращение ARX-структур:
...
обратные перестановки. В данном случае SHUFFLE выполняет 
ротацию слов влево или вправо, без потери информации - операция обратима.

1. Ротация слов влево заменяется на ротацию слов вправо.
2. Операция циклического сдвига влево заменяется на циклический сдвиг вправо.
3. Сложение `+=` заменяется на вычитание `-=`.
4. Порядок действий меняется на обратный

Сборка и тестирование:
	$ gcc -O3 -march=native -o test chacha20_rev.c
	$ ./test
...
#define Nr 10 // число двойных раундов 10 -> 20 раундов
