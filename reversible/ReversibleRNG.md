...
* [reversible Xoroshiro-64/128](xoroshiro_rev.c)
* [reversible Xoshiro-256](xoroshiro_rev.c)
* [reversible PCG64](pcg_rev.c)
* [reversible MWC64/128](mwc128_rev.c)
...
Метод мы разберем на примере простых функций. Возможность вывода обратных преобразования для генераторов типа LFSR, LCG, PСG, MCG и MWC может быть показана аналитически, через существование обратных чисел в модульной арифметике и в полиномиальной арифметике, с использованием нередуцируемых полиномов. В тех случаях, когда преобразование выполняется по модулю простого числа - обратное число существует и единственно для каждого числа принадлежащего множеству.
...
Для нахождения обратного числа по модулю используется выражение $a^{-1} \equiv a^{p-2} \mod p$ и алгоритм модульного возведения в степень, Modular Exponentiation. Для модулей $2^{2^n}$ может использоваться [метод деления Ньютона-Рафсона](https://en.wikipedia.org/wiki/Division_algorithm#Newton%E2%80%93Raphson_division), $O(\log_2(N))$.


**Метод пропуска шагов**

Seekability, `skip`, `jump`, 
```math
x_{n+i} = \left( a^{i} x_n + \frac{c(a^{i}-1)}{a-1}\right) \bmod P
```
...
<!-- метод Шраге, Schrage's algorithm -->
...
**Обратный шаг генераторов XorShift**

Операции сдвига и циклического сдвига можно представить бинарной матрицей трансформации. Один шаг генератора представлен, как умножение матрицы на вектор состояния. Для бинарной матрицы трансформации существует обратная матрица. Матрицу можно возводить в степень, и это позволяет найти максимальный период. Пропуск шагов может быть основан на искусстве возведения в степень матрицы трансформации. Если генератор содержит множество операций циклического сдвига, такие операции обратимы через полиномиальное умножение или через последовательность обратных сдвигов. Операцию сдвига на один разряд можно представить, как матрицы сдвига влево или вправо: L, R. Сдвиг на несколько разрядов - возведение в степень матрицы сдвига. Матрица трансформации XorShift состоит из трех сдвиговых матриц:
```math
\begin{align}
T &= (I + L^a)\cdot(I + R^b)\cdot (I + L^c)\\
T^{-1} &= (I + L^c)^{-1}(I + R^b)^{-1}(I + L^a)^{-1}
\end{align}
```
Матрицы сдвигов и вращений образуют некоммутативное кольцо. Обратные матрицы для $(I+L^a)$ можно представить как $(I+L^a)^{-1} \equiv (I+L^a)^k$, где $k=\lfloor 63/a \rfloor$. Формальное описание помогает синтезировать бинарную матрицу трансформации или разложить матрицу на компоненты сдвигов и вращений.

Один из основных инструментов исследования линейных трансформаций является характеристический полином. Если M - матрица трансформации, то характеристическим полиномом называется 
```math
𝑝(𝑥) = \det(𝑀 − 𝑥𝐼).
```
Соответствующий генератор будет иметь полный период $2^n-1$, если его полином является неприводимым, не раскладывается на множители в $\mathbb{F}_2$. Таким образом методом перебора можно найти параметры функции генератора, при которых генератор дает полный период. Характеристический полином можно представить бинарным вектором, числом размером $k$ - слов. Неприводимый полином можно возводить в степень без повторов $2^n-1$. Это дает критерий для отсева кандидатов.

Таким образом два метода используются для оценки матриц трансформации: последовательное возведение в степень матрицы через возведение в квадрат (exponentiation by squaring), вычисление характеристического многочлена (бинарного вектора) от бинарной матрицы трансформации и возведение в степень полученных полиномов. 

Для генераторов с большим размером слова состояния (в частности Xoshiro и Xoroshiro) применяют обобщенную теорию характеристического полинома [4], когда матрицу трансформации можно представить, как блочную матрицу $k\times k$ блоков, с размером блока $w\times w$, где $w$ - размер слова (32 или 64-бит). Блоки, матрицы $w\times w$, образуют коэффициенты характеристического полинома блочной матрицы. 

Линейная трансформация не обязательно основывается на матрицах сдвигов и вращения. В более общем случае это будет произвольная обратимая матрица - матрица аффинного преобразования. Обратная матрица получается через возведение в степень матрицы трансформации в кольце. Критерий выбора матриц для максимального периода может быть также основан на свойствах характеристического полинома. 

* [[1](https://www.jstatsoft.org/article/view/v008i14)] Marsaglia, G. 2003. Xorshift RNGs. _Journal of Statistical Software_. 8, 14 (Jul. 2003), 1–6. \
DOI:https://doi.org/10.18637/jss.v008.i14.
* [[2](https://www.iro.umontreal.ca/~lecuyer/myftp/papers/f2lin.pdf)] Pierre L’Ecuyer and François Panneton, $\mathbb{F}_2$-Linear Random Number Generators
* [[3](https://www.jstatsoft.org/article/view/v011i05)] Brent, R.P. 2004. Note on Marsaglia’s Xorshift Random Number Generators. _Journal of Statistical Software_. 11, 5 (Aug. 2004), 1–5.\
DOI:https://doi.org/10.18637/jss.v011.i05.
* [[4](https://arxiv.org/pdf/1805.01407)] David Blackman and Sebastiano Vigna, Scrambled Linear Pseudorandom Number Generators
